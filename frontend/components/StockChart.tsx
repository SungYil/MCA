...
interface StockChartProps {
    ticker: string;
    data?: PricePoint[]; // Should be passed from parent now to avoid refetching inside or keep internal fetching? 
                        // Current implementation fetches internally. Let's keep it but maybe allow passing data or refactoring.
                        // Actually, previous implementation fetched inside. I should verify if I can easily filter.
                        // Let's keep fetching inside but fetch MAX range (1Y) and filter locally for speed.
    averageCost?: number;
}
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const API_URL = process.env.NEXT_PUBLIC_API_URL || `${ protocol }//${hostname}:8000`; // Dynamically determine, or better pass from parent or env.

// Fetch 1 Year by default
const res = await fetch(`${API_URL}/api/stocks/${ticker}/history`); // Internal API defaults to 1Y
if (res.ok) {
